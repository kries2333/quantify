...
def QA_fetch_okex_swap_kline_min(
    symbol,
    start_time,
    end_time,
    frequency,
    ...
    while (reqParams['to'] > start_time):
        ...
        klines = QA_fetch_okex_swap_kline_with_auto_retry(
            symbol,
            reqParams['from'],
            reqParams['to'],
            frequency,
...
def QA_fetch_okex_swap_kline_with_auto_retry(
    ...
    获取永续合约币对的K线历史数据。K线数据按请求的粒度分组返回，k线数据最多可获取200条(说明文档中2000条系错误)。
    限速规则：20次/2s
    HTTP请求 GET/api/swap/v3/instruments/<instrument_id>/history/candles
    """
    url = urljoin(
        OKEx_base_url,
        "/api/swap/v3/instruments/{:s}/history/candles".format(symbol)
    )
