...
def QA_fetch_okex_futures_kline_min(
    symbol,
    start_time,
    end_time,
    frequency,
    ...
    while (reqParams['to'] > start_time):
        ...
        klines = QA_fetch_okex_futures_kline_with_auto_retry(
            symbol,
            reqParams['from'],
            reqParams['to'],
            frequency,
...
def QA_fetch_okex_futures_kline_with_auto_retry(
    symbol,
    start_time,
    end_time,
    frequency,
    ...
    限速规则：20次/2s
    HTTP请求 GET/api/futures/v3/instruments/<instrument_id>/history/candles
    ...
    url = urljoin(
        OKEx_base_url,
        "/api/futures/v3/instruments/{:s}/history/candles".format(symbol)
    )
