...
from QAFetch.QAOkEx_Spot import QA_fetch_okex_kline_min
        ...
        if len(missing_data_list) > 0:
            ...
            for i in range(len(missing_data_list)):
                ...
                data = QA_fetch_okex_kline_min(
                    ...
                    frequency=frequency,
                    callback_func=QA_SU_save_data_okex_swap_callback
                )

def QA_SU_save_data_okex_swap_callback(data, freq):
    """
    ...
    print(start_time)
    path = str(start_time)
    save_csv(data, 'okex', path, 'BTC_USDT_1m', 'swap')

def QA_SU_save_okex_swap():
    QA_SU_save_okex_swap_min('60')